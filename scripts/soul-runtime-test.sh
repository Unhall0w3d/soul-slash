...
printf 'Provider: %s\n' "${SOUL_RUNTIME_PROVIDER:-unknown}"
printf 'Base URL: %s\n' "$base_url"
printf 'Model:    %s\n' "$model"
printf '\n'

printf 'Checking model listing...\n'
if curl -fsS "${base_url%/}/models" >/tmp/soul-models.json; then
  printf 'OK: %s/models\n' "${base_url%/}"
else
...
run_chat_test() {
  local test_mode="$1"
  local payload_file="/tmp/soul-chat-${test_mode}.json"
  local response_file="/tmp/soul-chat-${test_mode}-response.json"

...
check.call("top bar retains four styled primary destinations", html.index('id="chat-tab"') < html.index('id="timeline-tab"') && html.index('id="timeline-tab"') < html.index('id="self-improvement-tab"') && html.index('id="self-improvement-tab"') < html.index('id="music-tab"') && html.include?('class="tab"'))
...
check.call("bootstrap advertises primary and nested surfaces separately", facade.include?('"product_tabs" => ["Chat", "Project Timeline", "Self Improvement", "Creative Studios"]') && facade.include?('"self_improvement_surfaces" => ["Skill Studio", "Self Assessment", "Self Augmentation"]') && facade.include?('"creative_surfaces" => ["Music Studio", "Visual Studio"]'))

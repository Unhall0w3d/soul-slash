...
check.call("Administration menu groups timeline, backup, and guided maintenance", %w[timeline-tab backup-tab maintenance-tab].all? { |id| html.include?("id=\"#{id}\"") } && html.index('id="administration-menu"') < html.index('id="timeline-tab"') && html.index('id="timeline-tab"') < html.index('id="backup-tab"') && html.include?('id="timeline-panel"') && javascript.include?('timeline: "#timeline-panel"'))
...
check.call("bootstrap advertises primary and nested surfaces separately", facade.include?('"product_tabs" => ["Chat", "Self Improvement", "Creative Studios", "Administration"]') && facade.include?('"self_improvement_surfaces" => ["Skill Studio", "Self Assessment", "Self Augmentation"]') && facade.include?('"creative_surfaces" => ["Music Studio", "Visual Studio", "Mix Studio"]') && facade.include?('"administration_surfaces" => ["Project Timeline", "Backup & Recovery", "Guided Maintenance"]'))
check.call(

...
check.call("top bar retains five styled primary destinations", html.index('id="chat-tab"') < html.index('id="timeline-tab"') && html.index('id="timeline-tab"') < html.index('id="self-improvement-tab"') && html.index('id="self-improvement-tab"') < html.index('id="creative-tab"') && html.index('id="creative-tab"') < html.index('id="administration-tab"') && html.include?('class="tab"'))
...
check.call("Administration menu separates backup and guided maintenance", %w[backup-tab maintenance-tab].all? { |id| html.include?("id=\"#{id}\"") } && html.include?('id="maintenance-panel"') && javascript.include?('maintenance: "#maintenance-panel"'))
...
check.call("bootstrap advertises primary and nested surfaces separately", facade.include?('"product_tabs" => ["Chat", "Project Timeline", "Self Improvement", "Creative Studios", "Administration"]') && facade.include?('"self_improvement_surfaces" => ["Skill Studio", "Self Assessment", "Self Augmentation"]') && facade.include?('"creative_surfaces" => ["Music Studio", "Visual Studio"]') && facade.include?('"administration_surfaces" => ["Backup & Recovery", "Guided Maintenance"]'))

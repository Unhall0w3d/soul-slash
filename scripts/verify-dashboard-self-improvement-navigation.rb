...
check.call("top bar retains four styled primary destinations", html.index('id="chat-tab"') < html.index('id="self-improvement-tab"') && html.index('id="self-improvement-tab"') < html.index('id="creative-tab"') && html.index('id="creative-tab"') < html.index('id="administration-tab"') && html.include?('class="tab"'))
...
check.call("Administration menu groups timeline, topology, backup, and guided maintenance", %w[timeline-tab local-topology-tab backup-tab maintenance-tab].all? { |id| html.include?("id=\"#{id}\"") } && html.index('id="administration-menu"') < html.index('id="timeline-tab"') && html.index('id="timeline-tab"') < html.index('id="local-topology-tab"') && html.index('id="local-topology-tab"') < html.index('id="backup-tab"') && html.include?('id="local-topology-panel"') && javascript.include?('topology: "#local-topology-panel"'))
...
check.call("bootstrap advertises primary and nested surfaces separately", facade.include?('"product_tabs" => ["Chat", "Self Improvement", "Creative Studios", "Administration"]') && facade.include?('"self_improvement_surfaces" => ["Skill Studio", "Self Assessment", "Self Augmentation"]') && facade.include?('"creative_surfaces" => ["Music Studio", "Visual Studio", "Mix Studio"]') && facade.include?('"administration_surfaces" => ["Host Stewardship", "Project Timeline", "Local Topology", "Backup & Recovery", "Guided Maintenance"]'))
check.call(

...
check.call("panel provides bounded privacy-safe memory visualization", %w[memory-constellation memory-constellation-layout memory-lifecycle-layout memory-constellation-inspector].all? { |id| panel.include?(%Q[id="#{id}"]) } && observatory_javascript.include?("renderMemoryConstellation"))

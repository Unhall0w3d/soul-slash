...
check.call("Blender service returns the standard application lifecycle key", File.binread(File.join(root, "lib/soul_core/blender_scene_service.rb")).include?('{ "lifecycle_state" => lifecycle') && !File.binread(File.join(root, "lib/soul_core/blender_scene_service.rb")).include?('{ "lifecycle" => lifecycle'))
...
check.call("bounded numeric controls preserve the string application contract", files[:javascript].include?('bars: String(byId("visual-blender-bars").value)') && files[:javascript].include?('seed: String(byId("visual-blender-seed").value)') && !files[:javascript].include?('bars: Number(byId("visual-blender-bars").value)') && !files[:javascript].include?('seed: Number(byId("visual-blender-seed").value)'))
check.call("first-generation preview omits absent revision lineage", files[:javascript].include?("if (state.visualBlenderSourceSceneId) input.source_blender_scene_id = state.visualBlenderSourceSceneId") && !files[:javascript].include?("source_blender_scene_id: state.visualBlenderSourceSceneId"))
...
puts "Blender Scene A3 verifier complete: 13 checks passed"

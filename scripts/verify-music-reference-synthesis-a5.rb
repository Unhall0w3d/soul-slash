...
def proposal(**overrides)
  JSON.generate({
    ...
    "caption" => "Original art-rock pulse with clipped clean-guitar figures, elastic bass, and close dry drums. Begin with an exposed motif, add the rhythm section for restrained verses, then widen each refrain with a second guitar register. Keep the lead vocal centered and intelligible. Let the bridge thin to bass and rim clicks before the final refrain restores the band, raises the dynamic ceiling, and ends on a short unresolved instrumental tag.",
...
def fusion_proposal
  value = JSON.parse(proposal(
    ...
    "caption" => "Unified nocturnal funk-rock led by syncopated clean guitar, elastic bass, tight dry drums, and compact brass answers. Open with the guitar pocket, bring the vocal into a restrained verse, then let brass punctuate phrase endings. Widen each chorus with guitar voicing and counter-rhythmic horns. Drop to bass, voice, and rim clicks in the bridge, then restore the ensemble for an escalating final chorus and precise stop."
  ))
  value["roles"] = [
...
Dir.mktmpdir("soul-reference-synthesis-a5") do |root|
  ...
  check.call("profile synthesis separates a coherent sonic portrait from the temporal script", request.messages.first.fetch("content").include?("one coherent overall portrait") && request.messages.first.fetch("content").include?("separate lyrics value is the temporal script") && request.messages.first.fetch("content").include?("512 characters"))
  check.call("profile synthesis repeats exact response keys and types for local models", request.messages.first.fetch("content").include?("exactly these nine keys") && request.messages.first.fetch("content").include?("caption") && request.messages.first.fetch("content").include?("exclusions must be an array of strings") && request.messages.first.fetch("content").include?("use 4 rather than 4/4"))
...
Dir.mktmpdir("soul-reference-synthesis-caption-contract-a5") do |root|
  ...
  store.write_track(track_fixture)
  caption = "Progressive rock at 110 BPM in D minor and 4/4 time, driven by distorted guitar, technical bass, and hard dry drums. Begin with a clean motif, escalate into a dense final passage, and keep the production immediate and rhythmically exact."

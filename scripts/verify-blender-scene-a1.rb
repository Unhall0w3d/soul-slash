...
checks = run_check(checks, failures, "load valid template manifest and compute deterministic digest") do
  ...
  template_keys = %w[abstract liminal architectural audio_reactive bioluminescent_grove void_sanctuary signal_forge willow_fungal_grove orbital_campfire_study]
...
checks = run_check(checks, failures, "adapter has no forbidden runtime capabilities") do
  ...
  raise "curated library loader escaped the reviewed function" unless loader_scope&.include?("bpy.data.libraries.load")
  missing_required = [
    "def run(manifest",
...
checks = run_check(checks, failures, "template catalog is closed and exact") do
  ...
  unless (templates.keys - %w[abstract liminal architectural audio_reactive bioluminescent_grove void_sanctuary signal_forge willow_fungal_grove orbital_campfire_study]).empty?
    raise "catalog contains unknown templates: #{(templates.keys - %w[abstract liminal architectural audio_reactive bioluminescent_grove void_sanctuary signal_forge willow_fungal_grove orbital_campfire_study]).join(', ')}"
  end
end

...
check("browser remains free of recurring/background transports and same-origin", %w[setInterval WebSocket EventSource serviceWorker innerHTML].none? { |needle| js.include?(needle) } && ![html, css, js].any? { |source| source.match?(%r{https?://}) }, errors)
check("server uses only bounded request-scoped concurrency", server.include?("listener.accept") && server.include?("MAX_CONCURRENT_REQUESTS = 64") && server.include?("REQUEST_SLOT_WAIT_SECONDS = 2.0") && server.include?("@request_available.wait") && server.include?("close_and_join_requests") && %w[fork( daemon( Process.spawn].none? { |needle| server.include?(needle) }, errors)

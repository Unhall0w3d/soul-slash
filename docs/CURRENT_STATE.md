...
- **Self Augmentation** creates human-authored architecture proposals, exact allowed-file experiments in isolated worktrees, deterministic candidate dossiers, and external integration handoffs. An optional digest-bound GPT-OSS critique can review one existing proposal and record cited strengths, concerns, unknowns, and revision questions, but cannot decide Gate A1, select files, create a worktree, or produce an implementation plan. Self Augmentation cannot invoke Codex, merge, push, or deploy.

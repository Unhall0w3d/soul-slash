...
Status: human-approved C1 candidate; status timer installed and first
supervised Forge maintenance accepted. Another device mutation, reboot, commit,
or merge requires its own explicit authority.
...
- Added one reboot request followed by bounded reconnect checks and changed
...
- `SOUL_MAINTENANCE_REMOTE_LIVE=false` remains effective.
...
- Forge and Pi-hole maintenance use fixed unattended APT `-y` with
  `--force-confold`; review this policy before the first live run.
- Pi-hole maintenance includes fixed `/usr/local/bin/pihole -up`; review its
  installed-version behavior before the first live run.
...
a scheduled status-only systemd timer. The timer is installed; the remote live
gate is disabled except during an exact supervised transaction.
...
Live package update or reboot during candidate completion: one supervised Forge package transaction; no reboot
```
...
- [ ] Review the fixed Pi-hole APT and `pihole -up` vectors.
- [ ] Accept or change the `--force-confold` unattended configuration policy.
- [ ] Review the 45-minute command and bounded reconnect limits.
- [ ] Confirm Maven must continue through its existing A2/A3 gates.
...
- [x] Review dead-owner, empty-race, malformed-lock, and replacement-inode
  recovery behavior.
- [ ] Run one supervised reboot only after reviewing dependency impact.

...
- Reworked the existing loopback server from sequential handling to at most eight tracked request-scoped threads so an explicit cancellation request can reach the active stream. Excess requests receive `429`; shutdown closes sockets and joins threads.

...
- There is no job queue, detached worker, watcher, scheduler, retry loop, or continuation after a request disconnects.
...
Generation output is drained through a bounded foreground wait with a fixed deadline. Client abandonment, server shutdown, timeout, explicit cancellation, or task completion terminates the exact owned process group, releases its lease, and leaves either a published candidate or an inspectable partial failure directory. No generation may survive its request.

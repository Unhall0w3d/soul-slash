...
module SoulCore
  class ReadOnlySkillExecutionGate
    ...
    def evaluate_plan(plan, execute: false)
      ...
      when "execution_history_summary"
        history_summary(plan)
      when "downloads_inspect"
        downloads_inspect(plan)
      else
    ...
    def explain(message, execute: false, record_history: false)
      ...
      lines = ["Read-only skill execution gate", "skill_id: #{r.skill_id || 'none'}", "risk: #{r.risk || 'unknown'}", "confirmation_required: #{r.confirmation_required}", "executed: #{r.executed}", "status: #{r.status}", "message: #{r.message}", "history_recorded: #{!r.history_entry.nil?}", "blocked_by:"]
      r.blocked_by.empty? ? lines << "- none" : r.blocked_by.each { |b| lines << "- #{b}" }
      lines.join("\n")
    end
    private
    ...
    def history_summary(plan)
      ...
      successful(plan, {"ok"=>true,"skill_id"=>plan.skill_id,"total_entries"=>rows.length,"shown_entries"=>@history.summary(limit: 10)["shown"],"counts_by_status"=>counts_by_status.sort.to_h,"counts_by_skill"=>counts_by_skill.sort.to_h,"latest"=>rows.last})
    end
    def downloads_inspect(plan)
      downloads = File.join(Dir.home, "Downloads")
      exists = Dir.exist?(downloads)
      entries = exists ? Dir.children(downloads) : []
      files = dirs = hidden = total_bytes = largest = 0
      extensions = Hash.new(0)
      entries.each do |name|
        hidden += 1 if name.start_with?(".")
        ...
        if File.directory?(path)
          ...
          total_bytes += size
          largest = [largest, size].max
          ext = File.extname(name).downcase
          ext = "[no extension]" if ext.empty?
          extensions[ext] += 1
        end
      end
      successful(plan, {"ok"=>true,"skill_id"=>plan.skill_id,"path"=>"~/Downloads","exists"=>exists,"entry_count"=>entries.length,"file_count"=>files,"directory_count"=>dirs,"hidden_entry_count"=>hidden,"total_file_bytes"=>total_bytes,"largest_file_bytes"=>largest,"extensions"=>extensions.sort_by { |ext,count| [-count, ext] }.to_h,"privacy"=>"filenames omitted"})
    end
    def successful(plan, payload)

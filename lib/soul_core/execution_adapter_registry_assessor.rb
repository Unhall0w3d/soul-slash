...
module SoulCore
  class ExecutionAdapterRegistryAssessor
    ...
    def assess
      ...
      blockers << "Expected the required enabled adapters" unless data["enabled_count"].to_i >= 6
      ...
      {"ok"=>blockers.empty?,"assessment"=>"execution_adapter_registry","phase"=>57,"generated_at"=>Time.now.iso8601,"root"=>@root,"status"=>blockers.empty? ? "ready" : "blocked","registry"=>data,"blockers"=>blockers,"verification"=>{"has_five_enabled_adapters"=>data["enabled_count"].to_i>=5,"has_weather_adapter"=>@registry.enabled?("weather.report"),"downloads_cleanup_plan_enabled"=>@registry.enabled?("downloads.cleanup_plan"),"downloads_cleanup_plan_has_internal_handler"=>@registry.internal_handler_for("downloads.cleanup_plan")=="downloads_cleanup_plan","downloads_cleanup_plan_review_only"=>@registry.find("downloads.cleanup_plan")&.risk=="review_only","has_blocked_adapters"=>data["blocked_count"].to_i>=1}}
    end

  ...
  module ConfigurationSchema
    ...
    def build
      definitions = [
        ...
        setting("maintenance.a3_live", "SOUL_MAINTENANCE_A3_LIVE", :boolean, false, effect: "Keeps conditional maintenance reboot and one-shot restoration disabled until a supervised A3 gate.", risk: "Enabling permits one exact reviewed transaction to request a reboot after fixed updates and durable snapshot capture."),
        ...
        setting("maintenance.fleet.maven_address", "SOUL_FLEET_MAVEN_ADDRESS", :string, "local", effect: "Provides a display-only workstation address or label for the fleet dashboard."),
        setting("maintenance.fleet.maven_label", "SOUL_FLEET_MAVEN_LABEL", :string, "Maven", effect: "Provides the human-readable workstation label without changing its stable internal device identity."),

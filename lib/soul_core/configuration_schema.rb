  ...
  module ConfigurationSchema
    ...
    def build
      definitions = [
        ...
        setting("memory.embedding_endpoint", "SOUL_MEMORY_EMBEDDING_ENDPOINT", :url, "", allow_empty: true, effect: "Locates the optional approved-memory embedding endpoint.", risk: "The embedding client independently enforces loopback HTTP and bounded requests."),
        setting("memory.embedding_profile", "SOUL_MEMORY_EMBEDDING_PROFILE", :string, "", effect: "Names the exact local embedding profile bound into a disposable memory index."),
        setting("memory.embedding_dimensions", "SOUL_MEMORY_EMBEDDING_DIMENSIONS", :integer, 0, range: 0..1_024, effect: "Binds the expected embedding dimensions; zero disables semantic retrieval."),
        setting("memory.embedding_protocol", "SOUL_MEMORY_EMBEDDING_PROTOCOL", :enum, "ollama", values: %w[ollama soul_fixture], effect: "Selects the bounded request schema for the explicitly configured loopback embedding endpoint."),
        setting("memory.embedding_query_instruction", "SOUL_MEMORY_EMBEDDING_QUERY_INSTRUCTION", :string, "", effect: "Optionally formats only retrieval queries with one reviewed instruction; indexed memory documents remain unprompted."),
        ...
        setting("maintenance.fleet.witness_control_enabled", "SOUL_FLEET_WITNESS_CONTROL_ENABLED", :boolean, false, effect: "Promotes only the enrolled Debian Raspberry Pi record matching the configured Witness address and SSH alias into the fixed device controller.", risk: "Enabling exposes preview-gated Witness APT maintenance and reboot controls; live execution still requires SOUL_MAINTENANCE_REMOTE_LIVE."),
        setting("maintenance.fleet.witness_address", "SOUL_FLEET_WITNESS_ADDRESS", :string, "witness", effect: "Selects the exact enrolled address and display address for Witness.", risk: "Keep deployment-specific private addresses in ignored local configuration."),
        setting("maintenance.fleet.witness_label", "SOUL_FLEET_WITNESS_LABEL", :string, "Witness", effect: "Provides the human-readable label for the separately enrolled passive security telemetry Raspberry Pi."),
        setting("maintenance.fleet.witness_ssh_alias", "SOUL_FLEET_WITNESS_SSH_ALIAS", :string, "witness", effect: "Selects one exact literal owner-local SSH Host alias for Witness's fixed controller.", risk: "The alias must refer to the separately reviewed operator maintenance identity; request data can never override it."),
        ...
        setting("maintenance.fleet.lattice_enabled", "SOUL_FLEET_LATTICE_ENABLED", :boolean, false, effect: "Adds the explicitly configured Netgear managed switch to bounded read-only SNMP fleet inventory.", risk: "Each SNMP query is restricted to one exact private IPv4 target; no SNMP SET or switch mutation authority is created."),
        setting("maintenance.fleet.lattice_address", "SOUL_FLEET_LATTICE_ADDRESS", :string, "", effect: "Selects the exact private IPv4 address for the optional Lattice switch inventory.", risk: "Keep deployment-specific addresses in ignored local configuration."),
        setting("maintenance.fleet.lattice_web_url", "SOUL_FLEET_LATTICE_WEB_URL", :string, "", effect: "Provides the optional private HTTP management URL displayed by the Dashboard.", risk: "The link carries no credentials and grants no management authority to Soul."),
        setting("maintenance.fleet.lattice_expected_firmware", "SOUL_FLEET_LATTICE_EXPECTED_FIRMWARE", :string, "", effect: "Records the operator-reviewed firmware version used for a local installed-version comparison.", risk: "Soul does not automatically browse, download, or install switch firmware."),
        setting("maintenance.fleet.lattice_snmp_community", "SOUL_FLEET_LATTICE_SNMP_COMMUNITY", :secret, nil, effect: "Authenticates bounded SNMPv2c read-only inventory for the optional Lattice switch.", risk: "Secret; stored only in owner-private ignored configuration and never returned by configuration or fleet evidence.", secret: true),
        setting("maintenance.fleet.loom_enabled", "SOUL_FLEET_LOOM_ENABLED", :boolean, false, effect: "Adds the explicitly configured Cisco managed switch to bounded read-only SNMP fleet inventory.", risk: "Each SNMP query is restricted to one exact private IPv4 target; no SNMP SET or switch mutation authority is created."),
        setting("maintenance.fleet.loom_address", "SOUL_FLEET_LOOM_ADDRESS", :string, "", effect: "Selects the exact private IPv4 address for the optional Loom switch inventory.", risk: "Keep deployment-specific addresses in ignored local configuration."),
        setting("maintenance.fleet.loom_web_url", "SOUL_FLEET_LOOM_WEB_URL", :string, "", effect: "Provides the optional private HTTP management URL displayed by the Dashboard.", risk: "The link carries no credentials and grants no management authority to Soul."),
        setting("maintenance.fleet.loom_expected_firmware", "SOUL_FLEET_LOOM_EXPECTED_FIRMWARE", :string, "", effect: "Records the operator-reviewed Loom firmware used for an installed-version comparison.", risk: "Soul does not automatically browse, download, or install switch firmware."),
        setting("maintenance.fleet.loom_snmp_community", "SOUL_FLEET_LOOM_SNMP_COMMUNITY", :secret, nil, effect: "Authenticates bounded SNMPv2c read-only inventory for the optional Loom switch.", risk: "Secret; stored only in owner-private ignored configuration and never returned by configuration or fleet evidence.", secret: true),
        ...
        setting("security.wazuh.posture_file", "SOUL_WAZUH_POSTURE_FILE", :string, "", effect: "Selects one ignored owner-private, versioned review of a raw Wazuh SCA result and exact classification of every reported failure.", risk: "This is an interpretation layer only; it never recalculates or replaces Wazuh's raw score and performs no remote query or mutation."),
        ...
        setting("voice.transcription.model", "SOUL_VOICE_TRANSCRIPTION_MODEL", :string, "ggml-base.en.bin", effect: "Selects one exact low-latency voice model filename declared by the transcription manifest; music vocal analysis retains its separate model selection."),

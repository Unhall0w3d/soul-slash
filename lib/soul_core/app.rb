  ...
  ok ? 0 : 1
    ...
    def run_assess
      ...
      when "phase11-bounded-artifact-inspection", "phase11-artifact-inspection", "artifact-inspection"
        ...
        puts(json ? JSON.pretty_generate(report) : assessor.render(report))
        report["ok"] ? 0 : 1
      when "phase11-artifact-metadata-attachment", "phase11-artifact-foundation", "artifact-foundation"
        json = @argv.include?("--json")
        assessor = Phase11ArtifactMetadataAttachmentAssessor.new(root: Dir.pwd)
        report = assessor.assess
        puts(json ? JSON.pretty_generate(report) : assessor.render(report))
        report["ok"] ? 0 : 1
    when "phase10-inspectable-interests-closeout", "phase10-closeout", "inspectable-interests"
      json = @argv.include?("--json")
      assessor = Phase10InspectableInterestsCloseoutAssessor.new(root: Dir.pwd)
      report = assessor.assess
      puts(json ? JSON.pretty_generate(report) : assessor.render(report))
      report["ok"] ? 0 : 1
    when "phase10-recent-style-awareness", "recent-style-awareness", "style-awareness"
      json = @argv.include?("--json")
      assessor = Phase10RecentStyleAwarenessAssessor.new(root: Dir.pwd)
      report = assessor.assess
      puts(json ? JSON.pretty_generate(report) : assessor.render(report))
      report["ok"] ? 0 : 1
    when "phase10-identity-style-foundation", "identity-style-foundation"
      json = @argv.include?("--json")
      assessor = Phase10IdentityStyleFoundationAssessor.new(root: Dir.pwd)
      report = assessor.assess
      puts(json ? JSON.pretty_generate(report) : assessor.render(report))
      report["ok"] ? 0 : 1
    ...
    when "phase9-reviewed-memory-controls", "reviewed-memory-controls"
      ...
      puts(json ? JSON.pretty_generate(report) : assessor.render(report))
      report["ok"] ? 0 : 1
    when "phase9-layered-memory-foundation", "layered-memory-foundation"
      json = @argv.include?("--json")
      assessor = Phase9LayeredMemoryFoundationAssessor.new(root: Dir.pwd)
      report = assessor.assess
      puts(json ? JSON.pretty_generate(report) : assessor.render(report))
      report["ok"] ? 0 : 1

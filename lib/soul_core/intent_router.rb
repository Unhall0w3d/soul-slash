...
module SoulCore
  class IntentRouter
    ...
    RULES = [
      ...
      ["repo_status", "Repository/runtime status", /(?:\A\s*(?:(?:repo|repository|runtime|soul)\s+)?(?:status|health)\s*[?.!]*\s*\z|\b(?:check|show|inspect|report|review|run|give me|tell me)\b.{0,45}\b(?:repo(?:sitory)?|runtime|soul)\b.{0,20}\b(?:status|health|condition)\b|\b(?:what(?:'s| is)|how(?:'s| is))\b.{0,35}\b(?:the\s+)?(?:repo(?:sitory)?|runtime|soul)\b.{0,20}\b(?:status|health)\b)/i, "system.status", "read_only", false, 0.78, "The request explicitly asks about project or runtime condition."],

...
module SoulCore
  class WitnessMaintenanceAuthority
    VERSION = "soul-witness-maintenance-a1-v1"
    CONFIRM_INSTALL = "INSTALL_WITNESS_MAINTENANCE_AUTHORITY"
    SSH_ALIAS = "witness"
    REMOTE_USER = "operator"
    REMOTE_HOSTNAME = "witness"
    HELPER_PATH = "/usr/local/libexec/soul-witness-maintenance"
    SUDOERS_PATH = "/etc/sudoers.d/90-soul-witness-maintenance"
    ...
    def plan
      ...
      basis = {
        "operation" => "witness_maintenance_authority_install",
        "version" => VERSION,
        "ssh_alias" => SSH_ALIAS,
        ...
        "allowed_operations" => %w[self-check apt-upgrade reboot],
      ...
      outcome(
        ...
        problems.empty?,
        problems.empty? ? "Review the exact Witness authority before installation." : problems.first,
        basis.merge(
    ...
    def status
      ...
      arbitrary = ssh("/usr/bin/sudo", "-n", "/usr/bin/id", timeout: 10)
      ready = parsed.is_a?(Hash) &&
        ...
        arbitrary.status != "ok"
      outcome("complete", true, "Witness maintenance authority status collected.", {
        "ready" => ready,
        "authority_mode" => ready ? "root_owned_fixed_operations" : "unavailable",
        "version" => parsed.is_a?(Hash) ? parsed["version"].to_s : "",
        "broad_cloud_init_authority_present" => parsed.is_a?(Hash) ? parsed["broad_cloud_init_authority_present"] : true,
        "arbitrary_passwordless_sudo" => arbitrary.status == "ok",
      ...
      outcome("complete", true, "Witness maintenance authority status collected.", {
        ...
        "arbitrary_passwordless_sudo" => true
      })
    end
    ...
    def install(expected_digest:, confirmation:)
      ...
      return outcome("awaiting_input", false, "Exact Witness authority confirmation is required.", data) unless confirmation == CONFIRM_INSTALL
      ...
      remote_helper = "/tmp/soul-witness-helper-#{token}"
      remote_sudoers = "/tmp/soul-witness-sudoers-#{token}"
      ...
      [
        ...
        raise "Witness authority install failed at #{File.basename(argv[2] || argv[0])}" unless result.status == "ok"
      end
      cleanup = ssh("/usr/bin/rm", "-f", remote_helper, remote_sudoers, timeout: 10)
      raise "Witness authority staging cleanup failed" unless cleanup.status == "ok"
      verified = status
      raise "Witness authority did not pass exact post-install verification" unless verified.dig("data", "ready")

      outcome("complete", true, "Witness root-owned maintenance authority installed exactly.", verified.fetch("data"), "privileged_policy_installed")
    ensure
    ...
    def helper_content
      File.binread(File.join(@root, "scripts", "soul-witness-maintenance-root"))
    end

    def sudoers_content(helper_digest = Digest::SHA256.hexdigest(helper_content))
      commands = %w[self-check apt-upgrade reboot].map do |operation|
        "sha256:#{helper_digest} #{HELPER_PATH} #{operation}"
      end.join(", \\\n    ")
      <<~SUDOERS
        # Soul/ Witness A1 fixed-operation maintenance authority. Generated; do not edit.
    ...
    def required_path_problems
      {
        ...
        "helper template" => File.join(@root, "scripts", "soul-witness-maintenance-root")
    ...
    def upload(local, remote)
      ...
      raise "Witness authority upload failed" unless result.status == "ok"
    end

    def write_temp(label, content, mode)
      path = File.join("/tmp", "soul-witness-authority-#{label}-#{Process.pid}-#{SecureRandom.hex(6)}")
